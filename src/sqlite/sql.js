...
class SQL {
  ...
  checkAndCreateUserTable(tableName, { userId, userName }) {
    if (tableName) {
      db.transaction(txn => {
        txn.executeSql(
          "SELECT name FROM sqlite_master WHERE type='table' AND name=?",
          ...
          (tx, res) => {
            ...
            if (res.rows.length == 0) {
              ...
              txn.executeSql(
                `CREATE TABLE IF NOT EXISTS ${tableName} (id INTEGER PRIMARY KEY AUTOINCREMENT, userId VARCHAR(50), userName VARCHAR(50), bankAmount NUMERIC, cashAmount NUMERIC)`,
                [],
              );
              txn.executeSql(
                `INSERT INTO ${tableName}(userId, userName, bankAmount, cashAmount) VALUES(?,?,?,?)`,
                [userId, userName, 0, 0],
                (tx, res) => {
